...
const buildOptions = {
  entryPoints: [path.join(__dirname, 'CopilotExtension.tsx')],
  bundle: true,
...
async function run() {
  if (isWatch) {
    ...
    await ctx.watch();
    console.log('[Watch] Watching for changes in Copilot...');
  } else {
    await esbuild.build(buildOptions);
    const stats = fs.statSync(path.join(__dirname, 'dist/main.js'));
    console.log(`[Build] Generated dist/main.js for Copilot (${(stats.size / 1024).toFixed(1)} KB)`);
  }
}
