 ...
 *
 * Adheres strictly to Noether theme CSS variables and zero transition delay rules.
 *
 * @author Yuliet Li
 * @since 1.0.0
 */
  ...
  const parsedNodes = useMemo(() => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.trim().startsWith('```')) {
        if (!inCodeBlock) {
          ...
          nodes.push(
            ...
            >
              <div className="flex items-center justify-between px-3 py-1.5 border-b border-[var(--noether-border-subtle,#242424)] bg-[var(--noether-bg-card,#1c1c1c)] text-[11px] text-[var(--noether-text-muted,#888888)] select-none">
                ...
                >
                  {isCopied ? (
                    <>
                      <CheckIcon size={12} className="text-[var(--noether-accent,#ea580c)]" />
                      <span className="text-[10px] text-[var(--noether-accent,#ea580c)]">Copied</span>
                    </>
                  ) : (
                    <>
                      <Copy01Icon size={12} />
              ...
              <pre className="p-3 text-[11px] font-mono leading-relaxed overflow-x-auto text-[var(--noether-text-primary,#f0f0f0)] select-text whitespace-pre">
      ...
      if (headingMatch) {
        ...
        if (level === 1) {
          nodes.push(
            <h2 key={`h1-${i}`} className="text-sm font-bold text-[var(--noether-text-primary,#ffffff)] mt-3 mb-1.5">
              <span dangerouslySetInnerHTML={{ __html: renderInlineTokens(text) }} />
            </h2>
          );
        } else if (level === 2) {
          nodes.push(
            <h3 key={`h2-${i}`} className="text-xs font-semibold text-[var(--noether-text-primary,#ffffff)] mt-2.5 mb-1">
              <span dangerouslySetInnerHTML={{ __html: renderInlineTokens(text) }} />
            </h3>
          );
        } else {
          nodes.push(
            <h4 key={`h3-${i}`} className="text-xs font-medium text-[var(--noether-text-primary,#e0e0e0)] mt-2 mb-0.5">
              <span dangerouslySetInnerHTML={{ __html: renderInlineTokens(text) }} />
            </h4>
          );
        }
...
function renderInlineTokens(raw: string): string {
  ...
  return (
    raw
      ...
      // Bold
      .replace(/\*\*([^*]+)\*\*/g, '<strong class="font-semibold text-[var(--noether-text-primary,#ffffff)]">$1</strong>')
      // Italics
      .replace(/\*([^*]+)\*/g, '<em class="italic text-[var(--noether-text-muted,#cccccc)]">$1</em>')
